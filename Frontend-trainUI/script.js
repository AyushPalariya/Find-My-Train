document.addEventListener('DOMContentLoaded', function () {
    ...
    // All stations from database for suggestions
    const allStations = [
        { name: 'New Delhi', code: 'NDLI' },
        { name: 'Mumbai Central', code: 'MUMB' },
        { name: 'Bengaluru City', code: 'BNGL' },
        { name: 'Hyderabad Deccan', code: 'HYD' },
        { name: 'Chennai Central', code: 'CHN' },
    ...
    setupAutocomplete(sourceCodeInput, sourceSuggestions);
    setupAutocomplete(destinationCodeInput, destSuggestions);

    // Close suggestions when clicking outside
    document.addEventListener('click', function(e) {
        if (!sourceCodeInput.contains(e.target) && !sourceSuggestions.contains(e.target)) {
            sourceSuggestions.style.display = 'none';
        }
        if (!destinationCodeInput.contains(e.target) && !destSuggestions.contains(e.target)) {
    ...
    // Functions
    function setupAutocomplete(inputElement, suggestionBox) {
    ...
    function searchTrains() {
        ...
        // API URL
        const apiUrl = `${Base_url}/train/search/byCode?sourceCode=${sourceCode}&destinationCode=${destinationCode}`;
